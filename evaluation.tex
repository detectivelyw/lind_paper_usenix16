...
which can be hard. Possibly remove ``our secure design'' and just keep ``Lind prototype''.}
...
\lois{Any particular reason for choosing these bugs programs?} 
\yiwen{I think we explained this in the experimental setup}
The kernel bugs
...
\lois{consequences of this?} 
\yiwen{the consequence is bypassing the intended file permission, which is a serious problem.}
...
\lois{Is the previous sentence needed?} 
\yiwen{possibly rewrite the sentence, but we want the readers to understand the importance of our study.}
...
\lois{Is it Unmount or umount. You have it both ways} 
\yiwen{one is for the system call function name, the other is just an English word.}
