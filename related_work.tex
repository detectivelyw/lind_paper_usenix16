...
This section discusses tools and techniques that share Lind's goals
to realize privileged code protection by enforcing strong
isolation between userspace and kernelspace.\lois{Do all of your subtopics
fall into this one category? If not, this introductory sentence should be
modified}
...
|lois{A general comment that really holds for the whole paper. If this draft is
 ...
 be more clearly delineated throughout the paper. I also think you have to
delineate how Lind differs from hypervisors}
...
8,000 LOC) enhances security compared to the above virtual machines. \lois{OK, what
does the TCB have to do with these languages?}
...
virtual machine-like interface to each process. \lois{How does this fit with
Lind?}
...
Garfinkel et al. in another effort, introduces Ostia \cite{SCI-04}, where system calls are delegated to
agents that enforce the policies. Ostia resolves the tight dependencies with the
underlying OS through process emulation and the agents model at the userspace.
Other issues in utilizing SCI solutions include the difficulty of appropriately
replicating OS semantics, multi-threaded applications that cause race conditions, or
indirect paths to resources that are often overlooked. \lois{This sentence is somewhat
incomplete and I do not know which Garfinkel paper it references. I think all the SCI comments
could be condensed into one sentence. Without knowing what is being referenced I am not
 sure whether I have properly cut things} In addition, system calls that are
denied can cause inconsistencies that are discussed in \cite{Problems-SCI}.
...
in the context of in-kernel interpreters such as Seccomp, which is used by several
 ...
 The authors propose Jitk architecture to avoid the weaknesses in traditional
  kernelspace system call interceptors that arise from the unforeseen bugs within
	...
	correct translation of the userspace submitted code, e.g. the  the BSD packet
	filter and INET-DIAG.
