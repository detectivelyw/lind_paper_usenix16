...
discern which practices actually improve security.\yanyan{why we need comparisons?}
...
environment provided by the small (8K LOC) sandbox kernel. This provides
straightforward access to the required common kernel paths while allowing more
complex functionality to be built.
As a result, Lind can offer enhanced security without losing basic functionality.
...
Furthermore, our results show that virtualization systems that use kernel
paths that are commonly executed, tend to have better resilience to vulnerabilities
