...
%Structurally, 
As shown in Figure \ref{diagram/fig3.png}, the Linux kernel is divided into several main sub-systems such as %sub-directories:
\texttt{kernel} (main kernel), \texttt{ipc} (inter-process communication), 
\texttt{net} (networking), \texttt{fs} (file system), and \texttt{device drivers} (device drivers). Chou %showed in~
